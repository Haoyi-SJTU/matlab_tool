...
运行大型程序通常需要较长时间，用户无法持续、及时关注运行结果（比如挂服务器上通宵跑代码）。所以在程序运行正确结束、或故障中断时调用这个函数，可以即时获取程序运行结果。

**功能**：向指定邮箱发送指定内容和标题的邮件。方便监测程序运行的结果
...
绘制有交互功能的plot图像和图例。用户可以自行勾选显示的图线，便于在数据复杂、图线重叠的情况下观察单个图线。
 ...
 - **函数调用**：`example_show_plotInteractiveData.m`
 - **交互界面**：
